############################################################
# Analyseskript zur Masterarbeit
#
# Demokratiequalität und Lebenszufriedenheit:
# Eine ländervergleichende Analyse der fünf Dimensionen des
# Economist Intelligence Unit Democracy Index (EIU-DI)
#
# Datenquellen:
#   - Daten.xlsx                  (EIU-DI 2025, WHR 2022-2024,
#                                  World-Bank-Kontrollen)
#   - social_support_whr2024.xlsx (Soziale Unterstützung, WHR)
#   - qog_std_ts_jan26.csv        (Quality of Government,
#                                  WGI-Indikatoren)
#   - V-Dem-CY-Core-v15.xlsx      (V-Dem-Indizes; Gegenprüfung)
#
# Output: alle Tabellen, Abbildungen und Diagnostiken werden
# nach <BASE_DIR>/out_analyse_<TIMESTAMP>/ geschrieben.
############################################################
...
# USER SETTINGS
# =========================
BASE_DIR <- getwd()  # Standard: aktuelles Arbeitsverzeichnis (für GitHub-Replikation)
MASTER_DEFAULT <- "Daten.xlsx"
SKIP_MAIN <- 1

SS_DEFAULT  <- "social_support_whr2024.xlsx"
QOG_DEFAULT <- "qog_std_ts_jan26.csv"

# V-Dem (separate Datei; wenn nicht vorhanden -> file.choose())
USE_VDEM <- TRUE
VDEM_DEFAULT <- "V-Dem-CY-Core-v15.xlsx"
VDEM_YEAR_MAX <- 2024        # nimm je Land das aktuellste Jahr <= VDEM_YEAR_MAX
VDEM_MODE <- "latest"        # "latest" | "align_to_whr" | "alt_period_2010_2015"
VDEM_ALIGN_YEARS <- 2021:2023

# N-Sicherung (nur EIU-Kernsample):
STOP_ON_N_MISMATCH <- TRUE
EXPECTED_N_FILE <- file.path(BASE_DIR, "EXPECTED_N_ANALYSIS_SAMPLE.txt")
# Wenn du absichtlich N ändern willst: STOP_ON_N_MISMATCH <- FALSE oder EXPECTED_N_FILE löschen.

# Bootstrap optional (kann dauern):
DO_BOOTSTRAP <- TRUE
B_BOOT <- 1000
...
# =========================
# 2) Arbeitsverzeichnis und Output-Ordner
# =========================
if (!dir.exists(BASE_DIR)) {
  message("BASE_DIR nicht gefunden. Nutze getwd(): ", getwd())
  BASE_DIR <- getwd()
}
setwd(BASE_DIR)

RUN_ID <- format(Sys.time(), "%Y%m%d_%H%M")
PATH_ROOT <- file.path(getwd(), paste0("out_analyse_", RUN_ID))
...
vc_cluster_from_data <- function(m, data, cluster_var){
  mf <- model.frame(m)
  idx <- suppressWarnings(as.integer(rownames(mf)))  # setzt voraus: rownames(data) sind 1..n
  if (anyNA(idx)) stop("Cluster-VCOV: rownames(model.frame) sind nicht numerisch. Prüfe rownames(data).")
...
# Standardisierung (z-Transformation) als globaler Helper
...
# =========================
# 4) Master-Daten einlesen
# =========================
main_file <- MASTER_DEFAULT
if (!file.exists(main_file)) {
  message("Master-Datei fehlt. Bitte wählen (xlsx/csv)…")
  main_file <- file.choose()
}
is_csv  <- grepl("\\.csv$",  main_file, ignore.case = TRUE)
is_xlsx <- grepl("\\.xlsx?$", main_file, ignore.case = TRUE)
if (!is_csv && !is_xlsx) stop("Bitte eine .xlsx oder .csv als Master-Tabelle auswählen.")
...
if (any(is.na(must))) {
  print(must)
  stop("Pflichtspalten fehlen. Bitte Spaltennamen prüfen oder find_col patterns erweitern.")
}
...
# =========================
# 5) WHR Social Support einlesen und joinen
# =========================
read_whr_social_support <- function(ss_file, iso3_ref = NULL){
  ...
  if (is.null(best)) stop("WHR social_support: Datei konnte nicht robust eingelesen werden (Header/Spalten prüfen).")
...
# =========================
# 6) QoG / WGI-6 Mittelwert 2022–2024
# =========================
...
read_vdem_raw <- function(vdem_file, iso3_ref = NULL){
  ...
  # xlsx: versuche skip 0..2 und wähle beste Variante
  best <- NULL
  best_score <- -Inf
  best_meta <- list()
  ...
  for (sk in 0:2){
    ...
    # "score": zählt wie viele Kernspalten vorhanden + iso-match
  ...
  if (is.null(best)) stop("V-Dem: Datei konnte nicht eingelesen werden (xlsx) oder hat unerwartetes Format.")
...
if (USE_VDEM) {
  ...
  vdem_mode_df <- function(mode = c("latest","alt_period_2010_2015","align_to_whr"),
    ...
    if (mode == "latest") {
      out <- base |>
        ...
        dplyr::ungroup() |>
        dplyr::select(-year)
    } else if (mode == "alt_period_2010_2015") {
      out <- base |>
  ...
  # Merge des in VDEM_MODE gewaehlten Modes
  ...
  if (!is.null(vdem_df) && nrow(vdem_df) > 0) {
    ...
    # Fail-safe: wenn alles NA nach Merge -> als "nicht verfügbar" behandeln
    if (all(dplyr::if_all(dat_mod[, vdem_vars, drop=FALSE], ~ is.na(.x)))) {
      warning("V-Dem Merge: Alle vdem_* Variablen sind NA. Prüfe vdem_read_log.txt / ISO3-Mapping.")
      vdem_vars <- character(0)
    }
  } else {
    warning("V-Dem konnte nicht verarbeitet werden (vdem_df NULL/leer). V-Dem Outputs werden übersprungen.")
  }
}
# =========================
# 8) Kontrollen und Kern-Sample (EIU)
# =========================
...
# Common sample: erzwinge identisches N über Kern-EIU-Modelle
...
# fehlende region/income nicht droppen -> "Unknown", damit FE/Cluster nicht abbricht
...
# Für Cluster-VCOV: numeric rownames setzen (ohne tibble-warning)
...
if (file.exists(EXPECTED_N_FILE)) {
  ...
  if (is.finite(N_exp) && !is.na(N_exp) && N_exp != N_now) {
    msg <- paste0("N mismatch: expected ", N_exp, " but got ", N_now,
                  ". Häufige Ursache: Merge (WHR/QoG) oder neue Controls.")
...
# =========================
# 9) Tabelle 2 (Deskriptive Statistik)
# =========================
...
# =========================
# 12) Simultaner EIU-Haupttest: 5 EIU-Dimensionen gemeinsam + Kontrollen
# =========================
if (length(eiu_dims) == 5) {

  # Gemeinsames EIU-Hauptmodell (alle fünf Subdimensionen gleichzeitig + gleicher Kontrollsatz)
  m_eiu_5 <- lm(mk_formula("life_satisfaction", c(eiu_dims, ctrl_vars)), data = dat_cc)

  # -------------------------
  # A) Haupttabelle als DOCX
  # -------------------------
  ...
  doc_eiu5 <- officer::read_docx() |>
    officer::body_add_par("Tabelle C3. Simultaner EIU-Haupttest: Alle fünf EIU-Subdimensionen + Kontrollen (OLS, HC3-robuste SE)", style = "heading 1") |>
    flextable::body_add_flextable(ft_eiu5)

  print(doc_eiu5, target = file.path(PATH_OUT, "TableC3_simultaneous_EIU_test.docx"))

  # -------------------------
  # B) Robuste Koeffizienten als CSV
  # -------------------------
  ...
  readr::write_csv(coefs_eiu5, file.path(PATH_OUT, "TableC3_simultaneous_EIU_test_coefficients.csv"))

  # -------------------------
  # C) Standardisierte Betas im simultanen Modell
  # -------------------------
  ...
  d_joint_std <- d_joint |>
    dplyr::mutate(dplyr::across(where(is.numeric), std_one))
  ...
  readr::write_csv(std_joint_df, file.path(PATH_OUT, "TableC3_simultaneous_EIU_test_plot_data.csv"))

  # -------------------------
  # D) Koeffizientenplot für den simultanen Haupttest
  # -------------------------
  p_eiu5 <- ggplot2::ggplot(std_joint_df, ggplot2::aes(x = beta, y = reorder(label, beta))) +
    ...
    ggplot2::labs(
      title = "Tabelle C3 (Visualisierung): Standardisierte Effekte der EIU-Subdimensionen im simultanen Haupttest",
      subtitle = "Alle fünf EIU-Subdimensionen gemeinsam mit identischem Kontrollsatz (HC3-robuste SE). Stützmaterial zur Tabelle C3.",
      x = "Standardisierter Koeffizient (beta)",
      y = ""
    )

  save_plot(p_eiu5, "TableC3_simultaneous_EIU_test_plot.png", w = 9.5, h = 5.8)

  # -------------------------
  # E) Vergleich: separat vs simultan
  # -------------------------
  ...
  readr::write_csv(compare_eiu_df, file.path(PATH_OUT, "TableC3_supplement_separate_vs_joint.csv"))

  # -------------------------
  # F) Zusatzdiagnostik für das gemeinsame EIU-Modell
  # -------------------------
  ...
  readr::write_csv(vif_joint_df, file.path(PATH_OUT, "TableC0_simultaneous_EIU_test_VIF.csv"))

  kappa_joint <- kappa(model.matrix(m_eiu_5), exact = TRUE)
  write_lines(
    c(
      "Tabelle C0: Multikollinearitätsdiagnostik des simultanen EIU-Haupttests",
      paste0("N = ", nobs(m_eiu_5)),
      paste0("Condition Number (kappa) = ", round(kappa_joint, 2)),
      "Hinweis: Berichtet im Fließtext Kap. 5.1; gehört konzeptionell zu Tabelle C0 / Anhang C."
    ),
    "TableC0_simultaneous_EIU_test_kappa.txt"
  )

} else {
  message("Simultaner EIU-Haupttest übersprungen: Nicht alle 5 EIU-Dimensionen vorhanden.")
}

# =========================
# 13) Tabelle 3b (Region-FE + region-clustered SE)
# =========================
if ("region" %in% names(dat_cc)) {
  ...
  # Kurze TXT-Zusammenfassung des Region-FE-Modells
...
# =========================
# 14) Abbildung 1 (Standardisierte Betas, EIU-Dimensionen)
# =========================
label_map <- c(
  electoral_process_pluralism = "Electoral process & pluralism",
  functioning_government      = "Functioning of government",
...
# =========================
# 16) Multikollinearitätsdiagnostik: VIF + Condition Number
# =========================
...
if (length(rhs_all) >= 2) {
  ...
  # Sensitivität: drop controls mit hohem VIF (>=5) und zeige EIU overall Effekt neu
...
# VIF für V-Dem-Indices (optional)
...
# Pearson-Korrelationsplot (corrplot-Stil)
...
# Pearson-Korrelations-Heatmap (ggplot-Stil; entspricht Abbildung D1 der Arbeit)
hm <- reshape2::melt(cP)
...
# =========================
# 18) Diagnostik: Residuen-Plots und Einflussdiagnostik
# =========================
...
# =========================
# 19) Robustheitsübersicht (Tabelle 4 / C2)
# =========================
# ---- Robust coefficient extraction (handles backticks, wrappers, renamed terms) ----
escape_regex <- function(x){
  # Escape regex metacharacters for safe use as literal text in regex patterns
  gsub("([][\\^$.|?*+(){}])", "\\\\\\1", x, perl = TRUE)
}
...
print(doc_roll, target = file.path(PATH_OUT, "Table4_Robustness_Rollup.docx"))

# =========================
# 20) Tabelle A2: OLS vs. robuste Regression (Dimensionen)
# =========================
...
# =========================
# 21) Tabelle A4: Jackknife (Leave-one-out)
# =========================
jack_all <- list()
...
# =========================
# 22) Tabelle A5: Exklusion der Top-10 nach Lebenszufriedenheit
# =========================
d_t10 <- exclude_top10(dat_cc)
...
# =========================
# 23) Ridge und Lasso (Tabelle R1 + CV-Plots)
# =========================
...
# =========================
# 24) PCA (Tabellen R2/R3 + Loadings-Plot)
# =========================
...
# =========================
# 25) Optionaler Bootstrap des EIU-Koeffizienten
# =========================
if (DO_BOOTSTRAP) {
...
# =========================
# 26) Koeffizientenplot: EIU-only vs. EIU + Kontrollen
# =========================
...
# =========================
# 27) V-Dem Robustheitsprüfungen (optional)
# =========================
if (USE_VDEM && length(vdem_vars) >= 3) {

  # Sample für V-Dem Analysen (nicht identisch zu EIU erzwingen, aber dokumentieren)
  ...
  p_vdem <- ggplot2::ggplot(Vtab, ggplot2::aes(x=reorder(.data$term, .data$estimate), y=.data$estimate))+
    ...
    ggplot2::coord_flip()+
    ggplot2::theme_minimal() +
    ggplot2::labs(x="", y="Koeffizient (HC3)", title="V-Dem Indizes (mit Kontrollen)")
  ...
  message("V-Dem Outputs übersprungen (zu wenige gemergte V-Dem Variablen oder zu kleines N).")
}



# =========================
# 28) Missingness-Diagnostik (Anhang A0; Kap. 4.1)
# =========================
excluded <- dat_mod |>
  dplyr::filter(
    !is.na(iso3c),
    !is.na(eiu_overall),
    !iso3c %in% dat_cc$iso3c
  )

# Stufentrennung der Ausschlüsse:
# Stufe (0a) = mindestens eine ökonomisch-strukturelle Kontrolle fehlt
#              (ln BIP/Kopf, Arbeitslosenquote, Altenquotient, Sozialschutzausgaben)
# Stufe (2+3) = ökonomisch-strukturelle Kontrollen vorhanden,
#               aber Social Support oder WGI fehlt
econ_ctrl_vars <- c("ln_gdp_pc_ppp", "unemployment_rate",
                    "old_age_dep", "social_protection_tot")
econ_ctrl_vars <- econ_ctrl_vars[econ_ctrl_vars %in% names(excluded)]

excluded <- excluded |>
  dplyr::mutate(
    n_missing_econ = rowSums(is.na(dplyr::across(dplyr::all_of(econ_ctrl_vars)))),
    stufe = dplyr::case_when(
      n_missing_econ > 0 ~ "(0a) Ökonomisch-strukturelle Kontrollen",
      TRUE ~ "(2+3) Social Support / WGI"
    )
  )

if ("region" %in% names(excluded) && any(!is.na(excluded$region))) {
  region_tab <- excluded |>
    dplyr::count(region, sort = TRUE) |>
    dplyr::rename(Region = region, N_excluded = n)
} else {
  excluded$wb_region <- countrycode::countrycode(
    excluded$iso3c, origin = "iso3c",
    destination = "region", warn = FALSE
  )
  region_tab <- excluded |>
    dplyr::count(wb_region, sort = TRUE) |>
    dplyr::rename(Region = wb_region, N_excluded = n)
}

# Hauptliste mit Stufen-Spalte
excluded_out <- excluded |>
  dplyr::select(country, iso3c, eiu_overall, stufe) |>
  dplyr::arrange(stufe, eiu_overall) |>
  as.data.frame()

readr::write_csv(excluded_out, file.path(PATH_OUT, "AppendixA0_excluded_countries.csv"))
readr::write_csv(as.data.frame(region_tab), file.path(PATH_OUT, "AppendixA0_excluded_by_region.csv"))

# Mittelwerts-Diagnostik nach Stufe
finale_eiu_mean <- mean(dat_cc$eiu_overall, na.rm = TRUE)

stage_summary <- excluded |>
  dplyr::group_by(stufe) |>
  dplyr::summarise(
    N           = dplyr::n(),
    mean_eiu    = mean(eiu_overall, na.rm = TRUE),
    sd_eiu      = sd(eiu_overall, na.rm = TRUE),
    min_eiu     = min(eiu_overall, na.rm = TRUE),
    max_eiu     = max(eiu_overall, na.rm = TRUE),
    .groups     = "drop"
  )

# Gesamt-Zeile
overall_summary <- tibble::tibble(
  stufe       = "Gesamt (alle Ausgeschlossenen)",
  N           = nrow(excluded),
  mean_eiu    = mean(excluded$eiu_overall, na.rm = TRUE),
  sd_eiu      = sd(excluded$eiu_overall, na.rm = TRUE),
  min_eiu     = min(excluded$eiu_overall, na.rm = TRUE),
  max_eiu     = max(excluded$eiu_overall, na.rm = TRUE)
)

# Finale Sample-Zeile
final_summary <- tibble::tibble(
  stufe       = "Finales Sample (dat_cc)",
  N           = nrow(dat_cc),
  mean_eiu    = finale_eiu_mean,
  sd_eiu      = sd(dat_cc$eiu_overall, na.rm = TRUE),
  min_eiu     = min(dat_cc$eiu_overall, na.rm = TRUE),
  max_eiu     = max(dat_cc$eiu_overall, na.rm = TRUE)
)

stage_summary_out <- dplyr::bind_rows(
  stage_summary, overall_summary, final_summary
) |>
  as.data.frame()

readr::write_csv(stage_summary_out,
                 file.path(PATH_OUT, "AppendixA0_stage_summary.csv"))

# =========================
# 29) Output-Manifest
# =========================
# Manifest aller Output-Files
...
# =========================
# 30) sessionInfo() dokumentieren
# =========================
sink(file.path(PATH_OUT, "sessionInfo.txt"))
print(sessionInfo())
sink()

message("sessionInfo() gespeichert: ", file.path(PATH_OUT, "sessionInfo.txt"))
